...
fs.readFile('data.txt', 'utf8' , (err, data) => {
    ...
    const count = lines.filter((line) => {
        ...
        let section1 = sections[0].split('-').map((item) => Number(item));
        let section2 = sections[1].split('-').map((item) => Number(item));

        return (
            (section1[0] >= section2[0] && section1[0] <= section2[1]) ||
            (section2[0] >= section1[0] && section2[0] <= section1[1]) ||
            (section1[1] <= section2[1] && section1[0] >= section2[0]) ||
            (section2[1] <= section1[1] && section2[0] >= section1[0])
        );
    }).length;
